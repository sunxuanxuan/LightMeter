...
测光模块负责从实时预览中估算当前场景的 `EV100_metered`；曝光计算模块负责结合用户设置的胶片 `ISO`、曝光补偿、1/3 档快门和光圈表，生成：
...
- `shutterStops`：可用快门档位。
- `frameFormat`：画幅，例如 135、6x4.5。
- `focalLength`：模拟焦段，例如 35mm、50mm、75mm。
...
- `ExposureStopTables`：维护 1/3 档快门和光圈表。
