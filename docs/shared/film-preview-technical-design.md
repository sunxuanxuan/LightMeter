...
Android 胶片预览模式的 $EV_{metered,100}$ 使用中央重点测光：中央矩形占有效
取景框面积 `30%`，保持相同宽高比并占线性亮度结果 `70%` 权重；外围区域占
剩余 `30%` 权重。
...
2. 使用同帧逐像素 EV、胶片宽容度和手机高光裁切标记生成风险图。
3. 使用固定相机参数 EV 作为模拟曝光参考。
4. 原子提交风险图和评级。

Android 当前采用单帧保守判定，不修改手机曝光，也不生成模拟 `+2 EV/-2 EV`
探测帧。固有纯白/纯黑与过曝/欠曝在单帧中可能无法区分，因此风险颜色表示
“超过阈值或信号裁切”，不表示确认存在可恢复细节。
