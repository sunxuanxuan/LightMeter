...
- 版本：V2.0 设计基线
- 适用平台：Android、iOS
- 产品定义：[产品需求文档](../product-requirements.md)
- 现有算法参考：[胶片曝光宽容度](film-exposure-latitude-reference.md)

本文定义一次性胶片相机预览能力的领域模型、计算公式、渲染管线、平台边界、测试
策略和实施顺序。拍立得支持应复用本文的曝光风险与建议管线，并补充其专属预设模型。

## 2. 核心原则

1. 手机相机负责采集场景和估算场景 EV，不直接冒充一次性胶片相机。
2. 一次性相机的 ISO、光圈和快门是固定预设，不写入手机相机曝光控制。
3. 模拟画面和驱动建议的曝光判断必须来自同一个不可变帧快照。
...
- 模拟画面和评级只能消费同一个 `FilmPreviewSnapshot`。
...
Android 胶片预览模式的 $EV_{metered,100}$ 使用有效取景框内的全局平均测光。
该模式对框内所有有效采样点采用同一权重，用于给出整体构图在固定相机参数下的
成片近似结果。
...
预设高光宽容度 $H$ 和暗部宽容度 $S$ 继续按 `1/3 EV` 保存。参与风险比较前，
分别将 $H$、$S$ 和像素差值的绝对值四舍五入到 `0.1 EV`：

```text
round10(abs(deltaEV)) <= round10(latitude) -> 不预警
deltaEV > 0 且超出边界                    -> 高光预警
deltaEV < 0 且超出边界                    -> 暗部预警
```

边界值本身视为安全。例如暗部边界显示为 `-1.7 EV` 时，`-1.7 EV` 不报警，
`-1.8 EV` 报警。越界后蒙层透明度随继续超出的档数增加；手机 YUV 已裁切时
直接使用最大强度高光预警。
...
专业模式和胶片预览模式共用该风险比较规则，区别仅在参考 EV：专业模式使用
测光结果与 EC，预览模式使用固定相机预设 EV。
...
$$
Y_{display}=0.18\times2^{0.5\Delta EV}
$$

因此 `±1.7 EV` 仍展示可见的曝光差异，但不会过早压暗或漂白画面。超过暗部
或高光宽容度后，以边界处的 `Y_{display}` 为起点，在额外 2 EV 内单调软裁切到
纯黑或纯白。禁止直接使用普通三次样条，因为过冲会破坏单调性。
...
Android 一次性相机冻结预览按上述压缩显示曝光与分段单调 `smoothstep` 实现。
超出宽容度边界后用 2 EV 的软延伸逐渐趋近纯黑或纯白。模拟器使用 OpenGL ES
对冻结 RGB 图逐原始像素计算响应曲线，输出宽高与冻结图完全一致；GPU 不可用时
回退到使用 LUT 的同尺寸 CPU 渲染。

专业模式不应用该胶片响应曲线，只模拟用户设置的曝光补偿：

$$
RGB_{professional}
=
clamp(RGB_{source}\times 2^{EC},0,1)
$$

运算在线性 RGB 中进行。`EC = 0` 时直接复制冻结原图，保证模拟结果逐像素不变；
正 EC 提亮，负 EC 压暗。
...
时不能恢复真实色彩，因此模拟结果必须配合自然语言建议说明可执行的调整动作。
...
左侧：同帧手机原图
右侧：同帧曝光响应模拟图
中间：可左右拖动的分割线和滑块，初始位置 50%
```

模拟生成期间保留冻结原图，并显示“正在生成成片模拟”。评级和宽容度判断仅用于
驱动冻结页下方的自然语言建议，不显示红绿风险蒙层、风险比例或眼睛切换按钮。

专业模式与胶片预览模式都只在按 `0.1 EV` 精度判断为越过宽容度边界后报警，
不再增加额外 `1/3 EV` 测量余量，也不从固定 `±1 EV` 提前报警。手机高光
裁切仍直接报警，不受宽容度比较影响。

专业模式冻结页同样提供圆形眼睛按钮，并只使用曝光补偿计算相对冻结原图的亮度
变化。宽容度仅用于风险蒙层，不参与专业模式模拟图的 tone mapping。按钮仅在
风险结果与模拟图均准备完成后显示；模拟视角不叠加风险蒙层和图例。
...
7. SceneRatingEngine 输出评级和原因码。
8. ViewModel 原子提交同一 frame token 的结果。
```
...
2. 使用同帧逐像素 EV、胶片宽容度和手机高光裁切标记计算评级与建议原因。
3. 使用固定相机参数 EV 作为模拟曝光参考。
4. 原子提交模拟图和评级。

Android 当前采用单帧保守判定，不修改手机曝光，也不生成模拟 `+2 EV/-2 EV`
探测帧。固有纯白/纯黑与过曝/欠曝在单帧中可能无法区分，因此模拟与建议必须
表达为近似结果，不表示确认存在可恢复细节。
