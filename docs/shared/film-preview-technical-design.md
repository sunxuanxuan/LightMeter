...
$$
risk_{highlight} = \Delta EV_{pixel} \ge H
$$

$$
risk_{shadow} = \Delta EV_{pixel} \le -S
$$

风险 Alpha 继续沿用现有“超过阈值后 2 EV 达到最大强度”的规则。手机 YUV 已
裁切时仍产生保守高光候选，但必须标记为手机信号边界。
