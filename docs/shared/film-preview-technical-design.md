...
Android 一次性相机冻结预览按上述三个锚点实现分段单调 `smoothstep`。在
`[-S, 0]` 和 `[0, H]` 内分别插值；超出宽容度边界后用 2 EV 的软延伸逐渐趋近
纯黑或纯白。模拟器使用 OpenGL ES 对冻结 RGB 图逐原始像素计算响应曲线，输出
宽高与冻结图完全一致；GPU 不可用时回退到使用 LUT 的同尺寸 CPU 渲染。
