...
- 实现状态：Android 已实现三通道独立 256 点 H&D LUT、响应前层混合、5-tap
  可分离高斯低通和密度相关颗粒；实测 LUT、MTF 标定与 halation 待后续迭代
- 关联文档：
...
sRGB Bitmap
  -> sRGB 解码
  -> 根据逐像素 Delta EV 重建 scene-linear RGB 曝光
  -> 3x3 感色层混合
  -> 分别计算 R/G/B 层 log exposure
  -> 三通道独立 256 点 H&D LUT
  -> 受限饱和度与密度相关颗粒
  -> sRGB 编码
```
...
- 在线性 RGB 中处理曝光。
- 18% 灰锚点稳定。
- 三通道分别经过连续、单调的 256 点响应 LUT。
- 层响应结果不再强制归一到统一目标亮度，保留曝光相关的通道亮度差异。
- GPU 渲染失败时有同尺寸 CPU 回退。

当前仍不能完整表达：

- 基于厂商曲线数字化或实拍标定的真实 H&D 响应；当前 LUT 仍由工程参数生成。
- 完整光谱敏感度、染料吸收和光源相关的颜色变化。
- 胶片 MTF 与乳剂内高光散射。
...
正式 profile 应由厂商 H&D 曲线数字化后执行单调重采样。当前 Android 实现在
数据尚未数字化时，使用 `toneGamma`、逐通道 `responseGamma` 和
`exposureBiasEv` 生成三条独立 LUT：

1. 趾部：在暗部宽容度内从 $D_{min}$ 平滑进入直线段。
2. 直线段：相对斜率由 `gamma / 0.64` 控制。`0.64` 是当前负片工程基准，
   归一后避免将负片密度 gamma 直接作为最终正像 gamma 而造成低反差。
...
- 使用 3 个带限噪声 octave，权重为 `0.55 / 0.30 / 0.15`，形成同时包含微粒、
  细颗粒和团簇的不规则结构；禁止逐像素独立白噪声。
...
| Profile ID | 当前显示名称/用途 | ISO | 暗部 / 高光 | tone | gamma RGB | bias RGB EV | sat | soft | grain | radius |
| --- | --- | ---: | ---: | ---: | --- | --- | ---: | ---: | ---: | ---: |
| `generic-color-100` | 通用 ISO 100 彩色负片 | 100 | -2 / +3 | 0.98 | 0.64 / 0.64 / 0.64 | 0 / 0 / 0 | 1.06 | 0.55 | 0.50 | 1.80 |
| `kodak-gold-200` | Kodak Gold 200 | 200 | -2 / +3 | 0.94 | 0.62 / 0.64 / 0.67 | +0.06 / 0 / -0.08 | 1.14 | 0.65 | 0.70 | 2.20 |
| `kodak-ultra-max-400` | Kodak Ultra Max 400 | 400 | -2 / +3 | 1.03 | 0.65 / 0.67 / 0.69 | +0.04 / 0 / -0.04 | 1.15 | 0.80 | 0.90 | 2.80 |
| `generic-color-800` | 通用 ISO 800 彩色负片 | 800 | -2 / +3 | 0.95 | 0.60 / 0.61 / 0.63 | 0 / 0 / 0 | 1.04 | 1.00 | 1.25 | 3.60 |
| `kodak-disposable-800` | FunSaver / Power Flash 内置 Kodak ISO 800 | 800 | -2 / +3 | 1.00 | 0.61 / 0.63 / 0.66 | +0.05 / 0 / -0.07 | 1.14 | 1.15 | 1.30 | 3.80 |
| `fuji-superia-xtra-400` | FUJICOLOR SUPERIA X-TRA 400 | 400 | -1.67 / +3 | 1.02 | 0.65 / 0.64 / 0.66 | -0.03 / +0.03 / +0.02 | 1.14 | 0.80 | 0.90 | 2.60 |
| `fuji-c400-400` | Fujifilm C400 ISO 400 彩色负片 | 400 | -2 / +3 | 1.00 | 0.63 / 0.64 / 0.66 | 0 / +0.02 / 0 | 1.12 | 0.80 | 0.95 | 2.80 |
...
2. [已完成] 实现参数化逐通道 H&D LUT。
3. [已完成] 改造 GPU 与 CPU 渲染器，并移除逐像素目标亮度强制归一化。
4. [已完成] 加入确定性、多尺度颗粒。
5. [部分完成] 加入 5-tap 可分离高斯低通；中频 acutance 与可关闭 halation
   待实现。
