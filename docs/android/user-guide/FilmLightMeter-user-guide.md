...
FilmLightMeter 需要输入设备绑定的签名凭证才能使用。App 全程不联网，凭证由
官网在确认付款后根据您的设备 ID 生成。

### 0.1 激活步骤

1. 首次启动 App 时，会进入激活页面。
2. 页面显示您的 **16 位设备 ID**（如 `ABBF 0883 E57D E7A0`）。
3. 点击设备 ID 旁的复制按钮，在官网订单页填写该设备 ID 并完成付款。
4. 付款成功后，官网会展示一段 **签名激活凭证**。
5. 将完整凭证复制到 App，点击 **激活**。
...
- 凭证与设备绑定，复制到其他设备无法使用。
- 更换手机或恢复出厂设置后，需通过原订单申请重新签发。
- 粘贴时必须保留完整凭证，不能删改其中字符。
...
- 测光圆圈仅在临时点测光时显示，常驻测光模式和机型预设不显示圆圈。
...
内置的机型预设用于模拟部分胶片相机的测光特性。Olympus 35 SP 提供
`平均（20°）` 和 `点测（6°）` 两个预设，选择后会同时应用对应的中央圆形
测光范围，但不会修改用户设置的焦段。选择 `自定义` 后，可手动调整当前模式的区域参数。
机型列表只显示与当前测光模式匹配的预设；选择机型后参数只读，切换回
`自定义` 后才显示范围和权重调节。
...
1. FilmLightMeter 使用中央重点测光保存当前画面和同一帧测光数据；中央 30%
   面积占 70% 权重。
2. 根据同帧 EV 和胶片宽容度绘制风险蒙层。
...
- 颜色越深，表示超出所选宽容度阈值越多。
- 宽容度按 `1/3 EV` 设置，风险判断精度为 `0.1 EV`；边界值本身不报警，
  超出下一个 `0.1 EV` 档位后报警。

算法采用单帧保守判定，不会修改手机相机曝光。纯白墙面、白纸、黑色头发或
黑色背包等固有明暗物体可能被标记为风险区域，应结合画面内容判断。
...
单帧信号无法可靠区分固有纯黑/纯白与欠曝/过曝。FilmLightMeter 会保守标记
超过胶片宽容度或手机高光裁切阈值的区域。

### 为什么冻结后风险图不是立即出现？

应用需要复制冻结 Bitmap、生成逐像素 ExposureMap 并在后台计算风险图和模拟
曝光结果，但不再等待额外曝光帧。
