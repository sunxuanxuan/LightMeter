...
测光模块负责从实时预览中估算当前场景的 `EV100_metered`；曝光计算模块负责结合用户设置的胶片 `ISO`、曝光补偿、1/3 档光圈表和 1/3 档快门表，生成：
...
1/4, 1/3, 0.4s, 1/2, 0.6s, 0.8s,
1s, 1.3s, 1.6s, 2s, 2.5s, 3.2s, 4s, 5s, 6s, 8s
```

显示采用相机常见标称值，内部秒数从 `1/2000s` 开始按 `2^(1/3)` 递增，确保
相邻候选严格相差 `1/3 EV`。
...
- `ExposureStopTables`：维护 1/3 档光圈表和 1/3 档快门表。
