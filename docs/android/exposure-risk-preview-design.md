...
低分辨率 Y 图的 `9×9` 邻域，并计算平均亮度和相邻像素平均梯度。窗口随风险
图从 240 px 提升到 480 px，以保持接近原实现的实际画面覆盖范围。仅在以下
条件同时成立时显示暗部风险：
...
单个探测采集最多等待 `800 ms`，但一次冻结从请求基准帧开始共享 `1350 ms`
硬截止，其中基准帧最多等待 `600 ms`。先扫描正常曝光图决定需要哪个方向，
避免无风险区域仍触发相机曝光切换。明暗候选同时存在时，从原始曝光直接切换
到两个目标补偿值；到达共享截止时间后停止等待、使用已有基准风险并统一恢复
曝光，禁止两个独立超时串行叠加。
