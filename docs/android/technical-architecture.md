...
焦距候选范围为 `20mm ~ 150mm`。进入专业界面或切换画幅时，系统根据手机
主摄光学参数和 CameraX 实际倍率上下限反算当前画幅可模拟的焦段区间，并与
候选范围求交，交集直接作为滑块范围。
...
冻结时不截取 `PreviewView`。UI 向 Analyzer 请求下一张有效分析帧，Analyzer
在关闭同一个 `ImageProxy` 前生成旋转、裁剪后的 Bitmap 和 ExposureSnapshot，
并以同一个时间戳和 revision 成对提交。Bitmap 转换只在冻结请求出现时执行，
不进入日常 10Hz 分析路径。
